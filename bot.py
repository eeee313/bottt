...
async def faq(interaction: discord.Interaction):
    ...
    embed.add_field(name="What happens if the other party doesn't cooperate?", value="Do not send any items or currency until your assigned middleman confirms the trade is ready to proceed on both sides. If a dispute arises, staff will review the ticket and any evidence provided before making a decision.", inline=False)
    embed.add_field(name="Can I open a ticket for something other than a trade?", value="Yes. General account or server issues should go through the support ticket panel, not the middleman panel, so they reach the correct team.", inline=False)
    embed.set_footer(text="Levi's Middleman Services")
    await interaction.response.send_message(embed=embed)
...
async def _create_ticket_channel(bot, interaction, category_id, allowed_role_names, ticket_type, name_prefix):
    ...
    try:
        ...
        await interaction.response.send_message("Failed to create ticket
